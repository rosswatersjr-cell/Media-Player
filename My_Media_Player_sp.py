...
def bind_keyboard():
    ...
    for k in range(len(keys)): 
        try:
            ...
            continue
def resize_fonts(base_font_sizes):# Calculate new font size relative to width
    new_font_sizes=[]
    base_width=1920
    screen_width=root.winfo_screenwidth()
    width=screen_width / base_width 
    scale_factor = ctypes.windll.shcore.GetScaleFactorForDevice(0) / 100
    for s in range(len(base_font_sizes)):
        new_size=int((base_font_sizes[s] * width) / scale_factor)
        if new_size < 8:new_size = 8
        new_font_sizes.append(new_size)
