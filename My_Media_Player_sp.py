...
class FFMPEG_Player():
    def __init__(self,parent):
        ...
        self.readme_path=os.path.join(pgm_path,"Bound Keys_sp.txt")
...
class AsyncTkApp:
    def __init__(self, root):
        ...
        self.quit_btn=tk.Button(self.ctrl_frame,text="Salir",foreground=F_Color.get(),image=self.btn_skin, compound="center",font=media_font2,relief="sunken",
        ...
        self.mute_btn=tk.Button(self.ctrl_frame,text="\U0001F50A",foreground=F_Color.get(),image=self.btn_skin, compound="center",font=emoji_font2,relief="sunken",
                        background="#bcbcbc",borderwidth=5,activeforeground="#4cffff",height=btn_hgt,width=1,justify="center",anchor="center")
        ...
        self.repeat_btn=tk.Button(self.ctrl_frame,text="🔁",foreground=F_Color.get(),image=self.btn_skin, compound="center",font=emoji_font2,relief="sunken",
                        background="#bcbcbc",borderwidth=5,activeforeground="#4cffff",height=btn_hgt,width=1,justify="center",anchor="center")
        ...
        self.shuffle_btn=tk.Button(self.ctrl_frame,text="🔀"+" ▶",foreground=F_Color.get(),image=self.btn_skin, compound="center",font=emoji_font2,relief="sunken",
                        background="#bcbcbc",borderwidth=5,activeforeground="#4cffff",height=btn_hgt,width=1,justify="center",anchor="center")
...
def set_screen_size():
    ...
    default=str((app.screen_height-app.root_height)+int(0.2*app.taskbar_height))
    msg1='Ingrese una Altura de Pantalla para la Reproducción de Video.\n'
    msg2=f"La Altura de Pantalla Predeterminada para este monitor = {default}.\n"
    msg3='La Altura Máxima para este monitor es  ' + str(app.work_area[3]) +' (Pantalla Completa).\n'
    msg4='¡El Ancho de la Pantalla será determinado por la relación de aspecto de este monitor!'
    msg=msg1+msg2+msg3+msg4
    hgt=my_askinteger(title,msg,Screen_Height.get(),100,app.work_area[3])
    if hgt!=None:
        Screen_Height.set(hgt)
        with open("Setup.json", 'r') as file:
            data = json.load(file)
...
def calculate_font_sizes():
    base_font_sizes = [10, 14, 18]
    new_font_sizes=[]
    scaling_factor = min(root.winfo_screenwidth() / 1920, root.winfo_screenheight() / 1032)  # Scale relative to 1920x1032  
...
def install_winget():
    urls = {"VCLibs": "https://aka.ms", "UI_Xaml": "https://github.com",  "WinGet": "https://github.com"}
    for name, url in urls.items():
        ps_command = f"Invoke-WebRequest -Uri '{url}' -OutFile '{name}.appx'; Add-AppxPackage -Path '{name}.appx'"
        try:
            subprocess.run(["powershell", "-Command", ps_command], check=True)
        except subprocess.CalledProcessError as e:
            title="< Instalación de WinGet >"
            msg1=f"¡La instalación de WinGet falló!\n"
            msg2="Es posible que necesite instalar WinGet manualmente.\n"
            msg3="¡Sin WinGet, no se pueden instalar los archivos de dependencias!\n"
            msg4=f"Error: '{e}'"
            msg=msg1+msg2+msg3+msg4
            messagebox.showerror(title,msg)
def is_winget_installed():
    try:
        if shutil.which("winget"):return True# WinGet' Executable Found in PATH
        # The WinGet --version' command is a reliable way to check its presence
        subprocess.run(
            ['winget', '--version'],
            check=True,
    ...
    try:#Installs SoundVolumeView using the winget package manager.
        ...
        for s in range(len(proc.stdout.splitlines())):
            if "Installed Location:" in proc.stdout.splitlines()[s]:
                winget_path = proc.stdout.splitlines()[s].replace("Installed Location: ","")
                break
        if winget_path == "":raise Exception("¡No se encontró la ubicación de instalación de SoundVolumeView!") 
        soundvolumeview_path.set(os.path.join(winget_path,"SoundVolumeView.exe"))
    except Exception as e:
        title="< Instalación de SoundVolumeView >"
        msg1=f"¡La instalación de SoundVolumeView falló!\n"
        msg2="Es posible que necesite instalar SoundVolumeView manualmente.\n"
        msg3="¡Sin SoundVolumeView, no se pueden seleccionar los dispositivos de salida de audio!\n"
        msg4=f"Error: '{e}'"
        msg=msg1+msg2+msg3+msg4
        messagebox.showerror(title,msg)
    ...
    try:#Installs FFmpeg using the winget package manager.
        ...
        for s in range(len(proc.stdout.splitlines())):
            if "Installed Location:" in proc.stdout.splitlines()[s]:
                winget_path = (proc.stdout.splitlines()[s]).replace("Installed Location: ","")
                break
        if winget_path == "":raise Exception("¡No se encontró la ubicación de instalación de FFmpeg!") 
        ffmpeg_path.set(os.path.join(winget_path,f"{version_info}/bin/ffmpeg.exe"))
        ffplay_path.set(os.path.join(winget_path,f"{version_info}/bin/ffplay.exe"))
        ffprobe_path.set(os.path.join(winget_path,f"{version_info}/bin/ffprobe.exe"))
    except Exception as e:
        title="< Instalación de FFmpeg >"
        msg1=f"¡La instalación de FFmpeg falló!\n"
        msg2="Es posible que necesite instalar FFmpeg manualmente.\n"
        msg3="¡Sin FFmpeg, no se pueden reproducir archivos multimedia!\n"
        msg4=f"Error: '{e}'"
        msg=msg1+msg2+msg3+msg4
        messagebox.showerror(title,msg)
if __name__ == '__main__':
    ...
    if len(data)==0:# Install Dependencies On First Execution
        if not is_winget_installed():install_winget()
