...
class AsyncTkApp:
    def __init__(self, root):
        ...
        #self.btn_skin = self.btn_skin.subsample(4, 2)# Resize Image
        ...
        self.quit_btn=tk.Button(self.ctrl_frame,text="Quit",foreground=F_Color.get(),image=self.btn_skin, compound="center",font=self.quit_font,relief="sunken",
        ...
        self.mute_btn=tk.Button(self.ctrl_frame,text="\U0001F50A",foreground=F_Color.get(),image=self.btn_skin, compound="center",font=self.mute_font,relief="sunken",
                        background="#bcbcbc",borderwidth=5,activeforeground="#4cffff",height=btn_hgt,width=1,justify="center",anchor="center")
        ...
        self.repeat_btn=tk.Button(self.ctrl_frame,text="🔁",foreground=F_Color.get(),image=self.btn_skin, compound="center",font=self.shuffle_repeat_font,relief="sunken",
                        background="#bcbcbc",borderwidth=5,activeforeground="#4cffff",height=btn_hgt,width=1,justify="center",anchor="center")
        ...
        self.shuffle_btn=tk.Button(self.ctrl_frame,text="🔀"+" ▶",foreground=F_Color.get(),image=self.btn_skin,compound="center",font=self.shuffle_repeat_font,relief="sunken",
                        background="#bcbcbc",borderwidth=5,activeforeground="#4cffff",height=btn_hgt,width=1,justify="center",anchor="center")
...
def set_screen_size():
    ...
    msg2=f"Default Screen Height For This Monitor = {default_height}.\n"
    msg3='Maximum Height For This Monitor is ' + str(max_height) +' (Full Screen).\n'
        ...
        if proc.stderr=="":
            ...
            if proc.stderr=="":
                ...
                for s in range(len(proc.stdout.splitlines())):
                    if "Installed Location:" in proc.stdout.splitlines()[s]:
                        pgm_path = (proc.stdout.splitlines()[s]).replace("Installed Location: ","")
                    elif "Version:" in proc.stdout.splitlines()[s]:
                        version = proc.stdout.splitlines()[s].replace("Version:","").replace(" ","")
                        version_info=f"ffmpeg-{version}-full_build"
                if pgm_path == "":# Restart And Check Again
                    ...
                    return True
            else:raise Exception("No Details!")    
        else:raise Exception("Installation Error!")
    except Exception as e:
...
def timed_popup(title, msg):
    ...
    popup_font = font.Font(family="Arial", size=20, weight="normal", slant="roman")
    popup_width = popup_font.measure(f"XXX{msg}XXX")
    font_metrics = popup_font.metrics()
    ...
    x = int(((screen_width // 2) - (popup_width // 2)) / scale_factor)
    y = int(((screen_height // 2) - (popup_height // 2)) / scale_factor)
    popup.geometry('%dx%d+%d+%d' % (popup_width, popup_height, x, y))
    popup.title(title)
    popup.lbl=tk.Label(popup, text=msg, font=popup_font, anchor="center", justify="center",  
                       borderwidth=2, foreground="#000000",background="#66ffff").pack(fill="both", expand=True)
    popup.update()
    popup.grab_release()
    time.sleep(2)
    popup.destroy()
if __name__ == '__main__':
    root=tk.Tk()
    sizes = calculate_font_sizes([12, 14, 18, 22, 24, 26])# Get Font Sizes Depending On Monitor Resolution
    ...
    emoji_font=font.Font(family='Noto Emoji', size=sizes[3], weight='normal', slant='roman')# All Other Buttons
    shuffle_repeat_font=font.Font(family='Noto Emoji', size=sizes[4], weight='normal', slant='roman')# Shuffle Repeat Buttons
    mute_font=font.Font(family='Noto Emoji', size=sizes[5], weight='normal', slant='roman')# Mute Button Text
    ...
    if len(data)==0:# Install Dependencies On First Execution
        ...
        try:
            ...
            if status:timed_popup(title="Installing WinGet", msg="WinGet Installation Successful!")
            else: raise Exception("Installing WinGet Failed!")
            status=winget_install_svv()
            if status:timed_popup(title="Installing SoundVolumeView", msg="SoundVolumeView Installation Successful!")
            else: raise Exception("Installing SoundVolumeView Failed!")
            status=winget_install_ffmpeg()
            if status:timed_popup(title="Installing Gyan FFmpeg", msg="Gyan FFmpeg Installation Successful!")
            else: raise Exception("Installing Gyan FFmpeg Failed!")
            mm_player.set_defaults()
            mm_player.restart_program(True)
        except Exception as e:
            title = "< Error Installing Dependency Files! >"
            msg1 = f"{e}\n"
            msg2="Exiting Program!"
            msg=msg1+msg2
            messagebox.showerror(title, msg)
