...
class FFMPEG_Player():
    def __init__(self,parent):
        ...
        self.ffprobe_path=os.path.join(pgm_path,"ffprobe.exe")
        self.ffplay_path=os.path.join(pgm_path,"ffplay.exe")
        self.soundvolumeview_path=os.path.join(pgm_path,"SoundVolumeView.exe")
    ...
    def restart_program(self,write):
        try:
            self.stop_process()
            cmd=[self.soundvolumeview_path, "/SetDefault", self.initial_sound_device, "1", "/Unmute", self.initial_sound_device, "/SetVolume", self.initial_sound_device, str(Level.get())]
    ...
    def set_defaults(self):
        ...
        if len(temp_dict)==0:
            ...
            Level.set(50.0)
            self.write_setup()
            mm_player.get_ffmpeg_versions()# Initialize ffmpeg files with PC Matic
        else:    
    ...
    def select_output_device(self,device):
        try:
            ...
            cmd=[self.soundvolumeview_path, "/SetDefault", soundview_device, "1", "/Unmute", soundview_device, "/SetVolume", soundview_device, str(Level.get())]
            subprocess.run(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            devices=AudioUtilities.GetSpeakers()# Initialize Master Volumn Slider
            self.interface = devices.EndpointVolume
            self.Master_Volume=cast(self.interface, POINTER(IAudioEndpointVolume))
...
if __name__ == '__main__':
    ...
    sizes = [-16, -18, -20, -24, -26]
    root.option_add("*Menu.Font", f"Helvetica {sizes[0]}")# Menu Dropdowns  
    media_font=font.Font(family='Times New Romans', size=sizes[0], weight='normal', slant='italic')# Play List
    quit_font=font.Font(family='Times New Romans', size=sizes[1], weight='normal', slant='italic')# Quit Button Text
