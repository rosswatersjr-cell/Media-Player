...
class YouTube_GUI(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.ReadMe_File.set(os.path.join(Path(__file__).parent.absolute(),"youtube_downloader_readme.txt"))
        self.youtube_readme=os.path.join(Path(__file__).parent.absolute(),"youtube_downloader_readme.txt")
...
def set_screen_size():
    ...
    msg3='Maximum Height For This Monitor is ' + str(app.work_area[3]) +' (Full Screen).\n'
...
def install_winget():
    ...
    for name, url in urls.items():
        ...
        try:
            ...
            msg1=f"WinGet Installation Failed!\n"
            msg2="You May Need To Manually Install WinGet.\n"
            msg3="Without WinGet, Dependency Files Cannot Be Installed!\n"
            msg4=f"Error: '{e}'"
            msg=msg1+msg2+msg3+msg4
            messagebox.showerror(title,msg)
    ...
    try:#Installs SoundVolumeView using the winget package manager.
        ...
        for s in range(len(proc.stdout.splitlines())):
            if "Installed Location:" in proc.stdout.splitlines()[s]:
                ...
                break
        if winget_path == "":raise Exception("No SoundVolumeView Installed Location Found!") 
        soundvolumeview_path.set(os.path.join(winget_path,"SoundVolumeView.exe"))
    except Exception as e:
        title="< SoundVolumeView Installation >"
        msg1=f"SoundVolumeView Installation Failed!\n"
        msg2="You May Need To Manually Install SoundVolumeView.\n"
        msg3="Without SoundVolumeView, Audio Output Devices Cannot Be Selected!\n"
        msg4=f"Error: '{e}'"
        msg=msg1+msg2+msg3+msg4
        messagebox.showerror(title,msg)
    ...
    try:#Installs FFmpeg using the winget package manager.
        ...
        for s in range(len(proc.stdout.splitlines())):
            if "Installed Location:" in proc.stdout.splitlines()[s]:
                winget_path = (proc.stdout.splitlines()[s]).replace("Installed Location: ","")
                break
        if winget_path == "":raise Exception("No FFmpeg Installed Location Found!") 
        ...
        msg1=f"FFmpeg Installation Failed!\n"
        msg2="You May Need To Manually Install FFmpeg.\n"
        msg3="Without FFmpeg, No Audio Or Video Files Can Be Played!\n"
        msg4=f"Error: '{e}'"
        msg=msg1+msg2+msg3+msg4
        messagebox.showerror(title,msg)
