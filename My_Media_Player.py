...
from pycaw.constants import CLSID_MMDeviceEnumerator, CLSID_CPolicyConfigClient
...
def install_ffmpeg_with_winget():
    #Installs FFmpeg using the winget package manager.
    try:
        ...
        proc = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, shell=True)
        winget_path = (proc.stdout.splitlines()[11]).replace("Installed Location: ","")
        #ffmpeg_path = "C:/Users/MiEsp/AppData/Local/Microsoft/WinGet/Packages/Gyan.FFmpeg_Microsoft.Winget.Source_8wekyb3d8bbwe/ffmpeg-8.0.1-full_build/bin/ffmpeg.exe"
        ...
        pass
def check_winget_exists():
    try:
        proc = subprocess.run(["winget", "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        version_info = (proc.stdout.splitlines()[0])
        return True
    except FileNotFoundError:
        # This exception is raised if the executable is not found in the PATH.
        return False
    except subprocess.CalledProcessError as e:
        # This might happen if the command runs but returns a non-zero exit code
        # for some other reason, though 'winget --version' should be reliable.
        return False
    except Exception as e:
...
if __name__ == '__main__':
    ...
    if not check_winget_exists():
        try:
            command = ["install", "-PackageProvider", "-Name","NuGet", "-Force | Out-Null"]
            proc = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        except: pass    
        try:
            command = ["install", "-Module", "-Name","Microsoft.WinGet.Client", "-Repository PSGallery | Out-Null"]
            proc = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        except: pass    
        try:
    ...
    with open('Setup.json', 'r') as file:
        data = json.load(file)
    if len(data)==0:mm_player.get_ffmpeg_versions()# Just To Initialize Executables On First Run
