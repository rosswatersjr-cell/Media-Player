...
class FFMPEG_Player():
    ...
    def get_ffmpeg_versions(self):
        try:
            ...
            result = subprocess.run(['ffplay', '-version'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    ...
    def get_duration(self,file):# minutes = "-sexagesimal", seconds = Blank
        try:
            proc=subprocess.Popen(["ffprobe.exe","-i",file,"-show_entries","format=duration","-v","quiet","-of","csv=p=0"], 
            ...
            if not self.ffplay_running:# Not Running
                ...
                try:
                    ...
                    if self.active_media=="video":
                        self.showmode_change=True
                        self.process_ffplay=subprocess.Popen(["ffplay","-ss",str(self.start_time),"-t",str(self.duration),"-x",window_wid,"-y",
                        ...
                        if title=="":# No title
                            ...
                            self.process_ffplay=subprocess.Popen(["ffplay","-ss",str(self.start_time),"-t",str(self.duration),"-x",
                                                                window_wid,"-y",window_hgt,"-autoexit",file,"-window_title",window_title],
                                                                stdin=subprocess.PIPE,stdout=subprocess.PIPE,creationflags=subprocess.CREATE_NO_WINDOW)
                        else:# title exist
                            self.showmode_change=True
                            self.process_ffplay=subprocess.Popen(["ffplay","-i","-ss",str(self.start_time),"-t",str(self.duration),"-x",window_wid,"-y",
