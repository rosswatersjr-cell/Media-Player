...
class YouTube_GUI(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        self.open_utube = ctk.CTkButton(self, text="Abrir Youtube", border_width=2, corner_radius=5, font=app.media_font, anchor="center",
        ...
        self.utube_readme = ctk.CTkButton(self, text="Ayuda de YouTube", border_width=2, corner_radius=5, font=app.media_font, anchor="center",
        ...
        self.conn_lbl = ctk.CTkLabel(self, text='Conexión a Internet: ',fg_color="transparent", 
        ...
        if not self.conn:
            self.retry_conn = ctk.CTkButton(self, text="Reintentar Conexión a Internet", border_width=2, corner_radius=5, 
        ...
        else: folder = "Descargas"
        self.Download_Folder.set(str(os.path.join(Path.home(),folder).replace("\\","/")))
        self.download_lbl = ctk.CTkLabel(self, text='Seleccionar Carpeta de Destino: ', fg_color="transparent",
        ...
        self.yt_user_lbl = ctk.CTkLabel(self, text='Nombre de Usuario de YouTube: ',fg_color="transparent", text_color=("#000000", "#ffffff"), font=app.media_font, anchor="e")
        ...
        self.oauth_btn = ctk.CTkCheckBox(self, text="Usa Autenticación OAuth",  border_width=2, variable=self.Use_Oauth,
        ...
        self.yt_pass_lbl = ctk.CTkLabel(self, text='Contraseña de YouTube: ',fg_color="transparent", text_color=("#000000", "#ffffff"), anchor="e", font=app.media_font)
        ...
        self.type_lbl = ctk.CTkLabel(self, text='Seleccionar Tipo de Descarga: ',fg_color="transparent", text_color=("#000000", "#ffffff"), anchor="e", font=app.media_font)
        ...
        self.url_lbl = ctk.CTkLabel(self, text='Introduce la URL de YouTube: ',fg_color="transparent", text_color=("#000000", "#ffffff"), anchor="e", font=app.media_font)
        ...
        self.fetch = ctk.CTkButton(self, text="Descargar", border_width=2, corner_radius=5, font=app.media_font, anchor="center",
        ...
        self.title_lbl = ctk.CTkLabel(self, text='Descargar Título: ', fg_color="transparent", text_color="#ffffff", anchor="e", font=app.media_font)
        ...
        self.progress_lbl = ctk.CTkLabel(self, text='Progreso de Descarga: ',fg_color="transparent", text_color=("#000000", "#ffffff"), anchor="e", font=app.media_font)
    ...
    def download_url(self,url, path, audio_video):
        ...
        if url is None or url=="":
            ...
            msg="¡URL Faltante! ¡Por Favor Ingresa una URL Válida!"
            MyDialog(parent=self, style="msgbox", title=title, prompt=msg, icon="cancel.png")
            self.url_txt.focus_force()
            return
        ...
        if not self.is_playlist(url):
            ...
            if not is_valid_link or link_type=='unknown':
                ...
                return
            if audio_video=="Solo Audio":# Single Audio
                if self.Use_Oauth.get():
            ...
            if audio_video=="Solo Audio":# All Audio 
                if self.Use_Oauth.get():
    ...
    def update_progressbar(self,d):
        if d['status'] == 'downloading':    
            ...
            if percentage>self.last_percentage:
                self.download_complete.configure(text=f"Descargando: {percentage:.2f}%")
            ...
            self.download_complete.configure(text=f"{self.completed_items} of {self.num_items} ¡Descarga Completada! 100%")
        elif d['status'] == 'error':
            self.last_percentage=0.0
            title="< Estado de la Descarga >"
            msg1="¡Error al Descargar de YouTube!\n"
    ...
    def change_download_folder(self,init_dir):#*
        folder_path=filedialog.askdirectory(initialdir=init_dir,title=f"¡Por Favor Selecciona una Carpeta para las Descargas de YouTube!")  
    ...
    def check_internet_connection(self,retry):#*
        try:
            ...
            if retry:
                ...
                self.conn_results.configure(text="  Hay una Conexión a Internet  ",foreground="#ffffff",background="#369e09")
                self.open_utube.configure(state="normal")
                self.update()
            return True
        except Exception:
...
class MyDialog(ctk.CTkToplevel):
    def __init__(self, parent, style: Literal["msgbox", "entry"], title, prompt, choices=None, 
        ...
        self.ok_button = ctk.CTkButton(self.button_frame, text="Vale", font=self.mydialog_font, command=self.on_ok)
        self.ok_button.pack(side="left", padx=10)
        self.cancel_button = ctk.CTkButton(self.button_frame, text="Cancelar", font=self.mydialog_font, command=self.on_cancel)
...
class App(ctk.CTk):#CustomTkinter window controlled by asyncio instead of mainloop().
    def __init__(self):
      ...
      # Define All Supported File Extensions
        ...
        self.title_txt=[f"{self.user}'s Reproductor de Medios:", "Haz Clic Derecho en la Lista de Medios para ver Opciones"]
        ...
        self.update_idletasks()
        self.Image_Size.set("Pantalla Normal")
        ...
        self.quit_btn = ctk.CTkButton(self.btn_frame, text="Salir", border_width=2, corner_radius=5, font=self.quit_font, anchor="center",
                    fg_color=("#a6e7ff", "#0c012e"), hover_color="#00ced1", text_color=("#0c012e", "#ffffff"))
    ...
    def get_taskbar_height(self):
        try:
            ...
            if not rc_monitor or not rc_work:
                raise RuntimeError("Información del Monitor no Disponible.")
    ...
    def get_greeting(self, status):
        ...
        if current_datetime.hour<12:
            if status=="open":greeting=f"¡Buenos Días {self.user} y Bienvenido!"
            else:greeting=f"¡Adiós {self.user}, que Tengas una Mañana Maravillosa!"    
        elif current_datetime.hour>=12 and current_datetime.hour<18:
            if status=="open":greeting=f"¡Buenas Tardes {self.user} y Bienvenido!"
            else:greeting=f"¡Adiós {self.user}, que Tengas una Tarde Maravillosa!"    
        elif current_datetime.hour>=18 and current_datetime.hour<=24:
            if status=="open":greeting=f"¡Buenas Noches {self.user} y Bienvenido!"
            else:greeting=f"¡Adiós {self.user}, que Tengas una Noche Maravillosa!"
        return greeting        
    def give_greeting(self, status):
        ...
        if status == "open":
            ...
            if current_datetime.hour<12:
                greeting=f"¡Adiós {self.user}. ¡Que Tengas una Maravillosa Mañana!"
            elif current_datetime.hour>=12 and current_datetime.hour<17:
                greeting=f"¡Adiós {self.user}. ¡Que Tengas una Maravillosa Tarde!"
            elif current_datetime.hour>=17 and current_datetime.hour<=24:
                greeting=f"¡Adiós {self.user}. ¡Que Tengas una Maravillosa Noche!"
        elif status == "restart":
    ...
    def set_defaults(self):
        ...
        if len(temp_dict)==1:# Only Theme Exist
            ...
            self.Screen_Position.set("Centro Superior")
    ...
    def config_video_library(self):
        title = "< Configurar Biblioteca de Videos >"
        msg = 'Selecciona la Tarea que Deseas Realizar en la Biblioteca de Videos.'
        args = ["Establecer la Biblioteca de Videos como Activa","Subir Carpeta a la Biblioteca de Videos","Agregar Archivos a la Biblioteca de Videos","Borrar Biblioteca de Videos"]
    ...
    def config_music_library(self):
        title = "< Configurar Biblioteca de Música >"
        msg = 'Selecciona la Tarea que Deseas Realizar en la Biblioteca de Música.'
        args = ["Establecer la Biblioteca de Música como Activa","Subir Carpeta a la Biblioteca de Música","Agregar Archivos a la Biblioteca de Música","Borrar Biblioteca de Música"]
        response = MyDialog(self, title=title, style="entry", prompt=msg, choices=args, init_val=None, icon="setup.png")
        if response.result in args:
            index = args.index(response.result)
            if index == 0:
    ...
    def config_image_library(self):
        title = "< Configurar Biblioteca de Imágenes >"
        msg = 'Selecciona la Tarea que Deseas Realizar en la Biblioteca de Imágenes.'
        args = ["Establecer la Biblioteca de Imágenes como Activa","Subir Carpeta a la Biblioteca de Imágenes",
                "Agregar Archivos a la Biblioteca de Imágenes","Borrar Biblioteca de Imágenes", "Configurar Retraso de la Presentación", 
    ...
    def set_screen_size(self):
        title = "< Configurar Tamaño de Pantalla de Video >"
        msg1 = 'Introduce una Altura de Pantalla para la Reproducción de Video.\n'
        msg2 = f"Altura de Pantalla Predeterminada para este Monitor = {self.default_height}.\n"
        msg3 = f'La Altura Máxima Dejando Visible la Barra de Tareas es {str(self.work_height)}.\n'
        msg4 = '¡El Ancho de la Pantalla será Determinado por\n'
        msg5 = 'la Relación de Aspecto de este Monitor!'
    ...
    def set_screen_position(self):
        title = "< Establecer Posición del Video >"
        msg1 = 'Selecciona una Posición en la Pantalla para\n'
        msg2 = 'Reproducir el Video.\n'
        msg3 = 'La Posición por Defecto es ' + self.Screen_Position.get()+'.'
        msg = msg1+msg2+msg3
        positions = ["Centro Superior","Arriba a la Izquierda","Arriba a la Derecha","Centro Izquierda","Centro","Centro Derecha","Abajo a la Izquierda","Centro Inferior","Abajo a la Derecha"]
    ...
    def select_audio_device(self, device=None):
        try:
            ...
            self.title(f"{self.title_txt[0]} Reproduciendo ({self.active_database.replace("_"," ")} Biblioteca), Reproduciendo en el Dispositivo de Audio: {result[0]} {self.title_txt[1].rjust(30+len(self.title_txt[1]))}")
        except Exception as ex:
            title='< Dispositivo de Salida de Audio >'
            msg1='Error al Inicializar el Dispositivo de Audio.\n'
            msg2=f"Error: '{ex}'\n"
            ...
            while not self._stop_timer:
                ...
                if self.paused:# self._factor Is Correction For Paused Time For Slider
                    ...
                    if remaining < 0.0:
                        ...
                        remaining = 0.0
                    self.Time_Now_Txt.set(f"Tiempo:(Total;{round(self.duration, 1)} seg., Transcurrido;{round(self._time_now, 1)} seg., Restante;{round(remaining, 1)} seg.)")
    ...
    def change_screen(self, screen_type, send_key=True):
        self.focus_force()
        try:
            if self.full_screen and screen_type == "pantalla normal":
                self.full_screen = False
            elif not self.full_screen and screen_type == "pantalla completa":     
    ...
    def set_slide_show(self):
        title='< Establecer el Tiempo de Retraso de la Presentación en Segundos >'
        msg1='Nota: ¡El Menú de Editar Imagen no se Muestra Cuando el\n'
        msg2='Retraso > 0.0! Introduce un Tiempo de Retraso en Segundos\n'
        msg3='para la Presentación de Diapositivas. Un Retraso Un Tiempo de\n'
        msg4='0.0 Segundos indica que no hay Presentación de Diapositivas,\n'
        msg5='pero las Imágenes Avanzará cada 5 Minutos para Proteger la\n'
        msg6='Pantalla. Mínimo El Tiempo de Retardo es de 0.5 Segundos y\n'
        msg7='el Máximo es de 30 Segundos.' 
    ...
    def set_color_theme(self, theme):
        title = " < Establecer Tema de Color >"
        msg = "Selecciona el Tema de Color que Quieras."
        if self.Theme.get() == "System":
            ...
            init_val = "Claro"          
        theme=MyDialog(self, title=title, style="entry", prompt=msg, choices=["Sistema", "Oscuro", "Claro"], init_val=init_val, icon="setup.png")
        "Sistema", "Oscuro", "Claro"
        if theme.result is not None:
            if theme.result == "Sistema":
                new_theme = "System"
            elif theme.result == "Oscuro":
                new_theme = "Dark"
            elif theme.result == "Claro":
                new_theme = "Light"          
    ...
    def show_menu(self, event):
        if not self.ffplay_running and not self.cv2_running:
            captions=["Configurar Biblioteca de Videos", "Configurar Biblioteca de Música", "Configurar Biblioteca de Imágenes", "Configurar Tamaño de Pantalla de Video", 
                      "Configurar Posición de la Pantalla de Video", "Configurar Tamaño de la Pantalla de la Imagen", "Seleccionar Dispositivo de Salida de Audio", "Abrir Descargador de Youtube", 
                      "Configurar Retraso de la Presentación", "Establecer Tema de Color", "Borrar Todas las Bibliotecas", "Ver Teclas del Teclado", "Acerca del Reproductor de Medios"]
            ...
            if self.Slide_Show_Delay.get() == 0.0:
                captions = ["Rotar Imagen y Guardar", "Eliminar Imagen de la Biblioteca", "Eliminar Imagen a la Papelera"]
            ...
            if self.full_screen:
                title = "Mostrar Pantalla Normal"
                txt = "pantalla normal"
            else:
                title = "Mostrar Pantalla Completa"
                txt = "pantalla completa"    
            captions = ["Mostrar Video/Arte", "Mostrar Olas", title]
    ...
    def set_master_volume(self, value):
        ...
        self.Volume_Now_Txt.set(f"Nivel: {int(float(value))} %  /  {round(db_level, decimals=2)} dB")
    def on_slide(self, value):
        try:
            ...
            pass
    def about(self):
        title = "< SOBRE EL REPRODUCTOR DE MEDIOS >"
        msg1='Creadora: Ross Waters'
        msg2='\nCorreo: RossWatersjr@gmail.com'
        msg3=f'\nRevisión: {version}'
    ...
    def init_dependencies(self):
        ...
        try:
            title = "< Iniciando FFMPEG >"
            msg1=f"No se pudo encontrar ffmpeg.exe. ¡Terminando el Programa!\n"
            ...
            title = "< Iniciando FFPROBE >"
            msg1=f"No se pudo encontrar ffprobe.exe. ¡Terminando el Programa!\n"
            ...
            title = "< Iniciando FFPLAY >"
            msg1=f"No se pudo encontrar ffplay.exe. ¡Terminando el Programa!\n"
            ...
            title = "< Inicializando SoundVolumeView >"
            msg1=f"No se pudo encontrar soundvolumeview.exe. ¡Terminando el Programa!\n"
            result = GetFileVersionInfo(self.soundvolumeview_path, '\\')
            ms = result['FileVersionMS']
            ls = result['FileVersionLS']
            version = f"{ms >> 16}.{ms & 0xFFFF}.{ls >> 16}.{ls & 0xFFFF}"
            title = "< Iniciando Deno >"
            msg1=f"No se pudo encontrar deno.exe. ¡Terminando el Programa!\n"
    ...
    def get_duration(self,file):# minutes = "-sexagesimal", seconds = Blank
        try:
            ...
            if err!='' or video_length=='' or proc.returncode!=0:# Try Different Approach
                ...
                video_length=output.decode()[:-1]
                err=(stderr.decode()[:-1])
                if err!='' or video_length=='' or proc.returncode!=0:raise Exception("ffprobe Obtener Duración del Stream")# Try Different Approach
    ...
    def bound_keys(self, key):
        if key.keysym=="XF86AudioPlay":self.ctrl_btn_clicked(self,"btn play")
            ...
            if self.full_screen:self.change_screen("pantalla normal")
            else:self.change_screen("pantalla completa")    
    ...
    def set_window_coord(self,wid,hgt):
        self.aspect_ratio=self.screen_width/self.work_height
        if self.Screen_Position.get()=="Arriba a la Izquierda":_x,_y=0,0
        elif self.Screen_Position.get()=="Centro Superior":_x,_y=int((self.screen_width/2)-(int(wid)/2)),0
        elif self.Screen_Position.get()=="Arriba a la Derecha":_x,_y=self.screen_width-int(wid),0
        elif self.Screen_Position.get()=="Centro Izquierda":_x,_y=0,int((self.work_height/2)-(int(hgt)/2))
        elif self.Screen_Position.get()=="Centro":_x,_y=int((self.screen_width/2)-(int(wid)/2)),int((self.work_height/2)-(int(hgt)/2))
        elif self.Screen_Position.get()=="Centro Derecha":_x,_y=int((self.screen_width)-(int(wid))),int((self.work_height/2)-(int(hgt)/2))
        elif self.Screen_Position.get()=="Abajo a la Izquierda": _x,_y=0,self.work_height-(int(hgt))
        elif self.Screen_Position.get()=="Centro Inferior": _x,_y=int((self.screen_width/2)-(int(wid)/2)),self.work_height-(int(hgt))
        elif self.Screen_Position.get()=="Abajo a la Derecha": _x,_y=int((self.screen_width)-(int(wid))),self.work_height-(int(hgt))
        else:
            self.Screen_Position.set("Centro Superior")
    ...
    def play_images(self, file, key):# Images/Photos etc...
        if self.next_ready:
            ...
            while self.listener.running and self.key_now!=None :
                try:
                    self.update_idletasks()
                    self.title_lbl.configure(text=f"Ahora en Cartelera: {os.path.basename(self.Media_Dict[str(self.key_now)])}")
                    ...
                    title="< Error al Mostrar Fotos/Imágenes >"
                    msg1="¡Algo Salió Mal!\n"
                    msg2=f"Error: {e}\n"
                    msg3="¡Quizás el Archivo Falta!\n"
                    msg4="¿Quieres Continuar a la Siguiente Foto/Imagen?"
                    msg=msg1+msg2+msg3+msg4
                    ...
                    if response:
                        ...
                        break
                try:
                    window_title=f"Reproductor de Medios: Reproduciendo {file}"
                    ...
                    if self.active_media=="image":
                        try:
                            if self.Image_Size.get() == "Pantalla Completa":
                                window_name = "Reproductor de Medios"
                                ...
                                cv2.setWindowTitle("Reproductor de Medios", window_title)
                                cv2.imshow("Reproductor de Medios", resized_img)
                            ...
                            title="< Error al Reproducir Imágenes >"
                            msg1="¡Algo Salió Mal!\n"
                            msg2=f"Error: {e}\n"
                            msg3="¡Quizás el Archivo está Perdido o Contaminado!\n"
                            msg4="¿Quieres Continuar con la Siguiente Imagen?"
                            msg=msg1+msg2+msg3+msg4
                            response=MyDialog(parent=self, style="msgbox", title=title, prompt=msg, icon="question.png")
                            if response:
                                cv2.destroyAllWindows()
    ...
    def play_av(self, file, key):# Audio/Video Files
        if self.next_ready:
            ...
            if not self.ffplay_running:# Not Running
                ...
                self.title_lbl.configure(text=f"Reproduciendo Ahora: {basename}")
                try:
                    ...
                    window_x,window_y=self.set_window_coord(window_wid,window_hgt)
                    window_title=f"Reproductor de Medios: Reproduciendo {self.active_file}"
                    ...
                    if self.active_media == "video":
                        self.showmode_change = True
                        self.formatted_time = self.minutes_to_hms(self.start_time)
                        self.process_ffplay = subprocess.Popen([self.ffplay_path, "-ignore_editlist", "1",
                                                                "-i", self.active_file, "-ss", self.formatted_time,
                                                                "-t", str(self.duration), "-x", window_wid, "-y", window_hgt,
                                                                "-showmode", self.show_mode, "-autoexit", "-window_title", window_title],
                        ...
                        if title=="":# No title
                            ...
                            self.process_ffplay = subprocess.Popen([self.ffplay_path,
                                                                    "-ss", str(self.start_time),
                                                                    "-t", str(self.duration),"-x", window_wid,"-y", window_hgt,
                                                                    "-autoexit", self.active_file,"-window_title", window_title],
                                                                    stdin=None, stdout=None, stderr=None,
                                                                    creationflags=subprocess.CREATE_NO_WINDOW)
                        else:# title exist
                            self.showmode_change = True
                            self.process_ffplay = subprocess.Popen([self.ffplay_path, 
                                                                    "-i", self.active_file, "-ss", str(self.start_time),
                                                                    "-t", str(self.duration), "-x", window_wid, "-y", window_hgt,
                                                                    "-showmode", self.show_mode, "-autoexit", "-window_title", window_title],
                                                                    stdin=None, stdout=None, stderr=None,
                                                                    creationflags=subprocess.CREATE_NO_WINDOW)
                    ...
                    if poll==None:# None = ffplay Running
                        ...
                        if self.full_screen:self.change_screen("pantalla completa")
                        self.ffplay_window.activate()
                        self._reset_timer()
                        self.Start_Time.set(perf_counter_ns())
                        self._start_timer()
                    else:raise Exception("ffplay no está Ejecutándose")
                except Exception as e:
                    title="< Error al Reproducir Audio/Video >"
                    msg1="¡Algo Salió Mal!\n"
                    msg2=f"Error: {e}\n"
                    msg3="¡Quizá el Archivo Falta o está Dañado!"
                    msg=msg1+msg2+msg3
                    response=MyDialog(parent=self, style="msgbox", title=title, prompt=msg, icon="question.png")
                    if response:
                        if self.ffplay_running:
                        ...
                        self.ffplay_running=False
                        self.next_ready=False
    def minutes_to_hms(self, minutes):
    ...
    def send_keyboard_key(self,key):
        ...
        elif key=="pantalla completa" or key=="pantalla normal":key_now=mykeys[8]
    ...
    def load_library(self, db, active_folder=None):
        ...
        if len(self.Media_Dict)==0:
            self.key_now=None
            title=f"< {db.replace("_"," ")} Biblioteca >"
            msg1=f'¡{db.replace("_"," ")} biblioteca está Vacía! Por favor, Selecciona\n'
            msg2='Primero "Subir Carpeta a la Biblioteca"'
            msg=msg1+msg2
            MyDialog(parent=self, style="msgbox", title=title, prompt=msg, icon="warning.png")
            return
        else:
            self.active_database=db
            self.title(f"{self.title_txt[0]} Reproduciendo ({self.active_database.replace("_"," ")} Biblioteca), Reproduciendo en el Dispositivo de Audio: {self.active_device} {self.title_txt[1].rjust(30+len(self.title_txt[1]))}")
    ...
    def rotate_image(self):
        ...
        msg1="Ingresa la Rotación de la Imagen en Grados.\n"
        msg2="¡El Rango es de -360 a 360 Grados!\n"
        msg3="Un Número Negativo Rota la Imagen en Sentido de las\n"
        msg4="Agujas del Reloj. Un Número Positivo Rota la Imagen\n"
        msg5="en Sentido Contrario a las Agujas del Reloj."
        msg=msg1+msg2+msg3+msg4+msg5
        ...
        if rotation.result is not None:
            ...
            try:
                ...
                title='< Rotación de Imagen >'
                msg1='¡Fallo al Rotar la Imagen!\n'
                msg2='¡Este Archivo de Imagen Puede Estar Dañado!\n'
                msg3=f"Error: '{e}'"
                msg=msg1+msg2+msg3
                MyDialog(parent=self, style="msgbox", title=title, prompt=msg, icon="cancel.png")
    def set_image_size(self):
        title = " < Establecer Tamaño de la Imagen Mostrada >"
        msg = "Selecciona el Tamaño de Imagen que Quieras."
        size=MyDialog(self, title=title, style="entry", prompt=msg, choices=["Pantalla Normal", "Pantalla Completa"], init_val=self.Image_Size.get(), icon="setup.png")
        ...
        try:
            if len(self.Media_Dict)>0:
                ...
                if os.path.exists(file_to_delete):
                    ...
                    title=f'< Eliminar Archivo de Imagen {file_name} >'
                    msg1=f'¡Archivo de Imagen {file_name}\n'
                    msg2='se Eliminó con éxito!'
                    msg=msg1+msg2
                    MyDialog(parent=self, style="msgbox", title=title, prompt=msg, icon="info.png")
    ...
    def remove_media_file(self, key=None, show_msg=None):
        try:
            if len(self.Media_Dict)>0:
                ...
                if show_msg:
                    if os.path.exists(file_to_remove):
                        title=f'< Eliminar Archivo de Imagen de la Base de Datos >'
                        msg1=f'¡El Archivo de Imagen {file_name}\n'
                        msg2='se Eliminó con éxito!'
                        msg=msg1+msg2
                        MyDialog(parent=self, style="msgbox", title=title, prompt=msg, icon="info.png")
                if self.active_media=="music" or self.active_media=="video":self.play_av(self.active_file,self.key_now)
            ...
            title=f'< Quitar Archivo de Imagen de la Base de Datos >'
            msg1=f'¡Error al Eliminar {file_name}!\n'
            msg2=f"Error: '{e}'"
            msg=msg1+msg2
            MyDialog(parent=self, style="msgbox", title=title, prompt=msg, icon="cancel.png")
            ...
            for j in range(0,len(file_names)):
                if file_paths[j]!="" and file_names[j]!="":
                    ...
                    if last_folder in db_list:# Add To Database
                        msgs.append(f"{file_names[j]} Fue Agregado a:\n"
                                     f"{file_paths[j]}.\n")
                        msg+=msgs[j]
    ...
    def add_files_to_db(self, db, files=None):
        ...
        if db=='Music':
            ...
            exts=image_exts
        if files==None:
            files=filedialog.askopenfilenames(title=f"Por Favor, Selecciona los Archivos para Subir a Base de Datos {db}.", filetypes=((type, exts),))
    ...
    def upload_from_folder(self, db, init_dir=None, ask=True):
        ...
        if ask:
            folder_path=filedialog.askdirectory(initialdir="C:\\", title=f"Por Favor, Selecciona una Carpeta para Subir a {db}.")  
            if folder_path=="" or folder_path==None:return
    ...
    def init_audio(self):
        try:
            ...
            self.Master_Volume.SetMasterVolumeLevelScalar(self.Volume_Level.get() / 100, None)
            db_level=self.Master_Volume.GetMasterVolumeLevel()
            self.Volume_Now_Txt.set(f"Nivel: {int(self.Volume_Level.get())} %  /  {round(db_level, decimals=2)} dB")
            self.muted=False
            self.paused=False
        except Exception as ex:
            title='< Inicialización de Audio >'
            msg1='¡Fallo en la Inicialización.\n'
            msg2='¡Terminando el Programa!\n'
            msg3=f"Error: '{ex}'"
            msg=msg1+msg2+msg3
            MyDialog(parent=self, style="msgbox", title=title, prompt=msg, icon="cancel.png")
            self.grab_release()
    ...
    def select_audio_device(self, device):
        try:
            ...
            self.Master_Volume=ctypes.cast(self.interface, ctypes.POINTER(IAudioEndpointVolume))
            self.Master_Volume.SetMasterVolumeLevelScalar(self.Volume_Level.get()/ 100, None)
            self.title(f"{self.user}'Reproductor de Medios: Reproduciendo ({self.active_database.replace("_"," ")} Biblioteca), Reproduciendo en el Dispositivo de Audio: {result[0]}")
        except Exception as ex:
            title='< Seleccionar Dispositivo de Salida de Audio >'
            msg1='¡Fallo al Seleccionar el Dispositivo de Audio!\n'
            msg2=f"Error: '{ex}'\n"
            msg3="Usando el Dispositivo de Audio Predeterminado."
            msg=msg1+msg2+msg3
            MyDialog(parent=self, style="msgbox", title=title, prompt=msg, icon="cancel.png")
